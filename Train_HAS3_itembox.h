...
SecureOTA ota(
    "https://raw.githubusercontent.com/Fuzzyline-HAS2/Train_HAS3_itembox/third_store/update.bin",
    "https://raw.githubusercontent.com/Fuzzyline-HAS2/Train_HAS3_itembox/third_store/version.txt",
    "https://raw.githubusercontent.com/Fuzzyline-HAS2/Train_HAS3_itembox/third_store/update.sig",
    HMAC_SECRET,
    FIRMWARE_VER
);
void DataChanged();
...
int modeValue[3][5] = { {160,90,70,50,0},     // VIBESTREGNTH (모터 세기): 0~255 설정 가능
                         {13,43,-1,-1,-1},      // ANSWER (Puzzle정답) 0 ~ 95 설정가능, -1 = 서버 자동해제, 서버값으로 갱신됨
...
bool pendingInnerEnable = false; // 서버 open 경로: BOX Opened 후 내부 태그(RfidLoopInner) 활성화 대기 - brownout 방지
unsigned long motorStartTime = 0;            // 모터 구동 시작 시각
const unsigned long motorOpenDuration = 4000; // 열기 모터 구동 시간(ms) - 스위치 미사용, 타이머로 정지
